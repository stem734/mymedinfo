## 2026-06-29 - Avoid Redundant Array Allocation and Lookups via Global Map Registry
**Learning:** React components that render tabs or menu links often define configuration arrays containing JSX elements (like icons) inside the component body. This causes allocation of the array and reconstruction of JSX nodes on every single render. Furthermore, searching these arrays with `.find()` inside loops runs in O(N*M) time. By moving these configurations to global constants and indexing them in a static `Map` outside the component scope, we achieve O(1) lookups and eliminate render-time overhead entirely. Memoizing derived statistics using `useMemo` additionally guarantees that typing in search filters does not run redundant O(N) filtering operations.
**Action:** Move static lists of links, tabs, and menu configuration metadata outside the React component scope. For key-based lookups, pre-build a static `Map` and access elements via `.get(key)` instead of `.find()`.
