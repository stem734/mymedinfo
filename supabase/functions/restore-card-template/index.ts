  ...
  try {
    ...
    if (builderType === 'medication') {
      if (payload.nhs_link && !isValidHttpUrl(payload.nhs_link)) {
        return errorResponse('NHS link in restored payload must be a valid HTTP or HTTPS URL', 400);
      }
      if (Array.isArray(payload.trend_links)) {
        for (const link of payload.trend_links) {
          const l = link as Record<string, unknown>;
          if (l.url && !isValidHttpUrl(l.url)) {
            return errorResponse('All trend links in restored payload must be valid HTTP or HTTPS URLs', 400);
          }
        }
      }
