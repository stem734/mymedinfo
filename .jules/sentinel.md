...
## 2025-06-17 - Multi-Dimensional Rate Limiting for Public Endpoints
**Vulnerability:** Publicly accessible endpoints like `send-password-reset` were vulnerable to mass-email abuse and brute-force enumeration because they lacked rate limiting. While email-based limits exist in some parts of the system, an attacker could bypass them using multiple emails from one IP or multiple IPs for one email.
**Learning:** Security-sensitive public endpoints require multi-dimensional rate limiting (IP + Identifier). A centralized, shared utility is essential for consistency and prevents "security theater" where some endpoints are protected and others are not. Returning success even when rate-limited preserves account enumeration safety.
**Prevention:** Implement a shared `recordAndCheckRateLimit` utility that tracks events in a dedicated `rate_limit_events` table. Enforce dual limits (e.g., 5/email/hour and 10/IP/hour) on all anonymous or high-risk endpoints.
