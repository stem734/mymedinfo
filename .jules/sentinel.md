...
## 2025-06-17 - Synchronizing User Deactivation with Supabase Auth
**Vulnerability:** The `update-practice-user` Edge Function updated the `is_active` status in the database but failed to synchronize this with Supabase Auth's `ban_duration`. This allowed deactivated practice users with active sessions to continue accessing the application until their session expired.
**Learning:** Application-level "active" flags in a database table are often insufficient for immediate access revocation if the authentication provider (like Supabase Auth) is not also notified. Banning the user in the Auth provider ensures that existing sessions are invalidated and new ones cannot be created.
**Prevention:** Always synchronize administrative deactivation/reactivation actions with the underlying authentication provider using `ban_duration` or equivalent mechanisms. Use magic values like `876600h` (100 years) for deactivation and `none` for reactivation, with explicit boolean checks to handle partial updates safely.
